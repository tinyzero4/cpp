...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        counts.fill(0);
        while (begin != end) {
            ++counts[static_cast<uint32_t>(*begin)];
            begin++;
        }
    }

    size_t Get(TAirport airport) const {
        return counts[index(airport)];
    }

    void Insert(TAirport airport) {
        counts[index(airport)]++;
    }

    void EraseOne(TAirport airport) {
        if (counts[index(airport)] > 0) counts[index(airport)]--;
    }

    void EraseAll(TAirport airport) {
        counts[index(airport)] = 0;
    }
    ...
    Items GetItems() const {
        ...
        for (int i = 0; i < counts.size(); i++) {
            items[i] = {static_cast<TAirport>(i), counts[i]};
        }
...
private:
    ...
    array<size_t, static_cast<uint32_t>(TAirport::Last_)> counts;

    uint64_t index(const TAirport airport) const {
...
void TestMostPopularAirport() {
    ...
    for (int day = 0; day < days_to_explore; ++day) {
        ...
        most_popular[day] = max_element(begin(items), end(items),
...
void TestCustom() {
    ...
    airport_counter.Insert(MoscowAirport::DME);
...
int main() {
    ...
    {
        LOG_DURATION("Total Total");

        {
            LOG_DURATION("Total TestMoscow");
            RUN_TEST(tr, TestMoscow);
        }
        {
            LOG_DURATION("Total TestManyConstructions");
            RUN_TEST(tr, TestManyConstructions);
        }
        {
            LOG_DURATION("Total TestManyGetItems");
            RUN_TEST(tr, TestManyGetItems);
        }
        {
            LOG_DURATION("Total TestMostPopularAirport");
