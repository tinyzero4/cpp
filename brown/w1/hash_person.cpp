...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        size_t prime = 13;
        return prime * prime * shash(address.city) + prime * address.building + shash(address.street);
    }
    hash<string> shash;
};
...
struct PersonHasher {
    size_t operator() (const Person& person) const {
        size_t prime = 13;
        return prime * prime * prime * ahash(person.address) + prime * prime * shash(person.name) + prime * person.height + dhash(person.weight);
    }
    hash<string> shash;
    hash<double> dhash;
    AddressHasher ahash;
