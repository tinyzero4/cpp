...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) :buckets_(vector<Bucket>(bucket_count))  {}

    WriteAccess operator[](const K& key) {
        Bucket &bucket = buckets_[hasher(key) % buckets_.size()];
        bucket.m.lock();

        if (bucket.data.count(key) == 0) {
            bucket.data[key] = V();
        }

        return {bucket.data.at(key), bucket.m};
    }

    ReadAccess At(const K& key) const {
        const Bucket &bucket = buckets_[hasher(key) % buckets_.size()];
        bucket.m.lock_shared();

        if (bucket.data.count(key) == 0) throw out_of_range("no key exist");
        return {bucket.data.at(key), bucket.m};
    }

    bool Has(const K& key) const {
        const Bucket &bucket = buckets_[hasher(key) % buckets_.size()];
        shared_lock guard(bucket.m);
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (int i = 0; i < buckets_.size(); i++) {
            ...
            for (auto const&[k, v] : bucket.data) {
                res[k] = v;
            }
        }
        return res;
...
void TestConstAccess() {
    ...
    for (int i = 0; i < 10; ++i) {
        futures.push_back(async([&cm, i] {
            try {
                const ConcurrentMap<int, string, hash<int>>::ReadAccess &access = cm.At(i);
//                access.ref_to_value = "";
                return access.ref_to_value;
            } catch (exception&) {
                return string();
            }
        }));
